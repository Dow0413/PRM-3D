...
// 动态障碍点云（单楼层）。不做圆盘拟合：局部观测只扫到障碍物的边角时，
// 拟合出的圆心会把禁行区放错位置。直接保留过滤后的障碍点（像素坐标），
// planFused 按同层边到最近点的距离切边/软惩罚 —— 参考 safeplanner-2
// PRMrtPlanner::updateDynamicObstacles + computeDynamicEdgeCost 的多楼层版
// （每层一份点集，只影响本层边，即「把动态障碍加到对应的地图中」）。
struct DynamicObstacleCloud
{
    int floor = 0;
    std::vector<BIpoint> points;   // 像素坐标（floor 字段冗余，统一设为本楼层）
    double forbidPx = 0.0;         // 硬禁行：边到最近点 < 此值 → 切边（含膨胀）
    double safetyPx = 0.0;         // 软避障：间隙 < 此值 → 按距离加权
};
...
public:
    ...
    // 静态可通行性（读原始 BIimap，不受 markObstacle 动态标记影响）。
    // 障碍点过滤用：落在静态障碍上的点不算动态障碍（safeplanner-2 规则）。
    bool isPointTraversableStatic(int floorIdx, double x, double y) const;
    ...
    // 动态障碍按点云传入（DynamicObstacleCloud，每层一份）：同层边到最近点的
    // 距离 < forbidPx 切边，间隙 < safetyPx 按贴墙惩罚公式加权。
    double planFused(MultiFloorTask& task, const std::vector<DynamicObstacleCloud>& dynClouds);
    // 融合参数：K = 在线插入的最近邻数；crossRadius = 多边形内跨层连边的距离阈值
    void setFuseParams(int kNeighbors, double crossRadius);
    // 贴墙惩罚：同层边权重 = 欧氏距离 × (1 + gain × (1 - c/clearancePx))，
    // c 为边上到最近(膨胀后)障碍的最小距离(px)；gain=0 关闭。
    void setWallPenalty(double gain, double clearancePx);
    const FusedGraph& getFusedGraph() const;
    const std::vector<TopoPolygon>& getTopoPolygons() const;
    bool fusedBuilt() const;

    // ====== 动态障碍（在线重规划用） ======
    // 把圆形障碍写入该楼层的可通行栅格（清零 _traversableCache 中的圆盘），
    // 之后 isPointTraversable / 线段碰撞都会绕开它；坐标与半径均为像素。
    bool markObstacle(int floorIdx, double px, double py, double radiusPx);
    // 从静态 BIimap 重建全部楼层的可通行缓存 —— 抹掉所有 markObstacle 的标记。
    // 障碍过期（obstacle_decay_sec）时：clearDynamicObstacles() 后重打仍存活的障碍。
    bool clearDynamicObstacles();
...
private:
    ...
    // 贴墙惩罚：每层每个像素到最近(膨胀后)障碍的欧氏距离(px)，由 distanceTransform 求得
    std::vector<std::vector<float>> _clearanceMaps;
    double _wallPenaltyGain = 0.0;           // 0 关闭
    double _wallPenaltyClearancePx = 10.0;   // 超过此 clearance 不惩罚(px)
    ...
    // 贴墙惩罚辅助：单点 clearance + 边上最小 clearance（均为像素）
    double clearanceAt(int floorIdx, double px, double py) const;
    double minClearanceAlongEdge(int floorIdx, const BIpoint& a, const BIpoint& b) const;
