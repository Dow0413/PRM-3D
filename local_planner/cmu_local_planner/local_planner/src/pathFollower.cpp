...
double maxYawRate = 45.0 * PI / 180.0;
double maxSpeed = 1.0;
double maxAccel = 1.0;
...
bool enableLateralVelocity = false;
double maxLateralSpeed = 0.0;
double maxLateralAccel = 0.0;
double lateralVelocityDirDiffThre = 0.7;
...
int main(int argc, char** argv)
{
  ...
  nh->declare_parameter<bool>("enableLateralVelocity", enableLateralVelocity);
  nh->declare_parameter<double>("maxLateralSpeed", maxLateralSpeed);
  nh->declare_parameter<double>("maxLateralAccel", maxLateralAccel);
  nh->declare_parameter<double>("lateralVelocityDirDiffThre", lateralVelocityDirDiffThre);
  ...
  nh->get_parameter("enableLateralVelocity", enableLateralVelocity);
  nh->get_parameter("maxLateralSpeed", maxLateralSpeed);
  nh->get_parameter("maxLateralAccel", maxLateralAccel);
  nh->get_parameter("lateralVelocityDirDiffThre", lateralVelocityDirDiffThre);

  if (maxLateralSpeed <= 0.0) maxLateralSpeed = maxSpeed;
  if (maxLateralAccel <= 0.0) maxLateralAccel = maxAccel;
  if (lateralVelocityDirDiffThre <= 0.0) lateralVelocityDirDiffThre = dirDiffThre;
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      float vehiclePlanarSpeed = fabs(vehicleSpeed);
      if (enableLateralVelocity) {
        vehiclePlanarSpeed = sqrt(vehicleSpeed * vehicleSpeed + vehicleLateralSpeed * vehicleLateralSpeed);
      }

      if (vehiclePlanarSpeed < 2.0 * maxAccel / 100.0) vehicleYawRate = -stopYawRateGain * dirDiff;
      ...
      if (joySpeed2 == 0 && !autonomyMode) {
        vehicleYawRate = maxYawRate * joyYaw;
      ...
      if (enableLateralVelocity) {
        ...
        if (pathSize > 1 && dis > stopDisThre && fabs(dirDiff) < lateralVelocityDirDiffThre) {
          float targetDirInVehicle = -dirDiff;
          targetSpeedX = joySpeed3 * cos(targetDirInVehicle);
          targetSpeedY = clampAbs(
            joySpeed3 * sin(targetDirInVehicle),
            static_cast<float>(maxLateralSpeed));
        }
        vehicleSpeed = approachValue(vehicleSpeed, targetSpeedX, maxAccel / 100.0);
        vehicleLateralSpeed = approachValue(
          vehicleLateralSpeed,
          targetSpeedY,
          maxLateralAccel / 100.0);
      ...
      if (pubSkipCount < 0) {
        ...
        if (enableLateralVelocity && fabs(vehicleLateralSpeed) > maxLateralAccel / 100.0) {
