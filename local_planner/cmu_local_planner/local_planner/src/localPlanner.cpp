...
double pathHysteresisScoreRatio = 0.85;
...
void obstacleHeightThresholdHandler(const std_msgs::msg::Float32::ConstSharedPtr threshold)
{
  if (!std::isfinite(threshold->data) || threshold->data < 0.0) {
    RCLCPP_WARN(
      nh->get_logger(),
      "Ignoring invalid obstacle height threshold: %.3f",
      threshold->data);
    return;
  }

  static double lastLoggedThreshold = -1.0;
  obstacleHeightThre = threshold->data;
  if (fabs(obstacleHeightThre - lastLoggedThreshold) > 1e-4) {
    RCLCPP_INFO(
      nh->get_logger(),
      "Obstacle height threshold set to %.3f m",
      obstacleHeightThre);
    lastLoggedThreshold = obstacleHeightThre;
  }
}
...
int main(int argc, char** argv)
{
  ...
  nh->declare_parameter<bool>("usePathHysteresis", usePathHysteresis);
  nh->declare_parameter<double>("pathHysteresisScoreRatio", pathHysteresisScoreRatio);
  ...
  nh->get_parameter("usePathHysteresis", usePathHysteresis);
  nh->get_parameter("pathHysteresisScoreRatio", pathHysteresisScoreRatio);
  ...
  if (pathHysteresisScoreRatio < 0.0) pathHysteresisScoreRatio = 0.0;
  else if (pathHysteresisScoreRatio > 1.0) pathHysteresisScoreRatio = 1.0;
  ...
  auto subObstacleHeightThreshold = nh->create_subscription<std_msgs::msg::Float32>(
    "/obstacle_height_threshold", 5, obstacleHeightThresholdHandler);
  ...
  while (status) {
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        if (usePathHysteresis && selectedGroupID >= 0 && lastSelectedCandidateID >= 0 &&
            lastSelectedCandidateID < 36 * groupNum) {
          int lastRotDir = int(lastSelectedCandidateID / groupNum);
          float lastRotAng = (10.0 * lastRotDir - 180.0) * PI / 180;
          float lastRotDeg = 10.0 * lastRotDir;
          if (lastRotDeg > 180.0) lastRotDeg -= 360.0;

          bool lastRotClear = ((lastRotAng * 180.0 / PI > minObsAngCW && lastRotAng * 180.0 / PI < minObsAngCCW) ||
                               (lastRotDeg > minObsAngCW && lastRotDeg < minObsAngCCW && twoWayDrive) || !checkRotObstacle);
          float lastScore = clearPathPerGroupScore[lastSelectedCandidateID];
          if (lastRotClear && lastScore > 0 && lastScore >= maxScore * pathHysteresisScoreRatio) {
            selectedGroupID = lastSelectedCandidateID;
          }
        }

        if (selectedGroupID >= 0) {
          lastSelectedCandidateID = selectedGroupID;
